...
t_point project_iso(t_point p, t_vars *vars)
{
    ...
    // ROTAÇÃO DE CÂMERA (não do objeto!)
    // Para rotacionar a câmera, aplico rotações INVERSAS no objeto
    // Ordem inversa: Z depois, Y e X antes
    t_point temp = rotate_z(centered, -vars->rotation.z);  // inverte Z
    temp = rotate_y(temp, -vars->rotation.y);  // inverte Y  
    t_point rotated = rotate_x(temp, -vars->rotation.x);  // inverte X
...
int update_and_render(t_vars *vars)
{
    ...
    // ROTAÇÃO DE CÂMERA (como videogame!)
    // W/S/A/D movem a câmera, não o objeto
    
    // Q/E: gira a câmera no eixo Z (rola a cabeça) - INVERTIDO
    if (vars->keys.q)
        vars->rotation.z -= step;
    if (vars->keys.e)
        vars->rotation.z += step;
    
    // W/S: move câmera verticalmente (olha para cima/baixo)
    if (vars->keys.w)
        vars->rotation.x += step;
    if (vars->keys.s)
        vars->rotation.x -= step;
    
    // A/D: move câmera horizontalmente (olha para esquerda/direita)
    if (vars->keys.a)
...
int main(int argc, char **argv)
{
    ...
    // Rotação inicial (isométrico padrão)
    vars.rotation.x = 0;
    vars.rotation.y = 0;
    vars.rotation.z = 0;
